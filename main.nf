...
def get_entry_point() {
    // Define the order of operations and their associated parameter
    def process_order = [
        [name: 'FASTP',    toggle: params.ENABLE_FASTP],
        [name: 'SGA',      toggle: params.ENABLE_SGA],
        [name: 'PRINSEQ',  toggle: params.ENABLE_PRINSEQ],
        [name: 'KRAKEN2',      toggle: params.ENABLE_KRAKEN_GTDB],
        [name: 'MAPPING',     toggle: params.ENABLE_MAPPING],
        [name: 'NGSLCA',      toggle: params.ENABLE_NGSLCA],
        [name: 'BAMDAM',      toggle: params.ENABLE_BAMDAM],
        [name: 'MMSEQS2',     toggle: params.ENABLE_MMSEQS2]
    ]

    // Find the first item where the toggle is "enable"
    def first_step = process_order.find { it.toggle == "enable" }

    return first_step ? first_step.name : "NOT FOUND"
}

// Usage
def startPoint = get_entry_point()
log.info "Workflow starting point identified as: $startPoint"
...
//         if (params.ENABLE_LOW_COMPLEXITY_FILTER=="PRINSEQ" && params.ENABLE_PRINSEQ != "enable") 
//             error "[Config Error] PRINSEQ is used instead of SGA for filtering out low-complexity reads. Please enable PRINSEQ."
        
// 		if (params.ENABLE_LOW_COMPLEXITY_FILTER=="SGA" && params.ENABLE_SGA != "enable") 
//             error "[Config Error] SGA is used instead of PRINSEQ for filtering out low-complexity reads. Please enable SGA."
...
// 	preprocessed_reads = ch_sample_ids.map { id -> [id, []] } // for nextflow evaluation
// 	fastp_ch = ch_sample_ids.map { id -> [id, []] }
...
// 	kraken_out = ch_sample_ids.map { id -> [id, []] }
// 	if (params.ENABLE_KRAKEN_GTDB == "enable") { 
...
// 	bowtie2_out = ch_sample_ids.map { id -> [id, []] }
// 	if (params.ENABLE_MAPPING == "enable") {
...
// 	} else {
// 		mmseq2_evaluation = ch_sample_ids.map { id -> [id, []] }
// 	}
...
// 		//version2------------------
// 		// input_metrics = Channel
// 		// .join(
// 		// 	paired_reads.map        { tuple(it[0], [paired_reads: it]) },
// 		// 	fastp_ch.map            { tuple(it[0], [fastp: it]) },
// 		// 	preprocessed_reads.map  { tuple(it[0], [preprocessed: it]) },
// 		// 	kraken_out.map          { tuple(it[0], [kraken: it]) },
// 		// 	mapped_bam.map          { tuple(it[0], [mapped: it]) },
// 		// 	bamdam_bam_lca.map      { tuple(it[0], [bamdam: tuple(it[0], it[1])]) }
// 		// )
// 		// .groupTuple()
// 		// .map { id, maps ->
// 		// 	def merged = maps.collectEntries()
// 		// 	tuple(
// 		// 		id,
// 		// 		merged.paired_reads,
// 		// 		merged.fastp,
