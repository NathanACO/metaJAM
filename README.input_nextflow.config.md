...
`KRAKEN2_BATCH_SIZE=128`: give the number as either the number of samples if it's less than the number of cores you gave to KRAKEN2 (see `withName: KRAKEN2 {` in the nextflow.config), or the number of cores required by Kraken2. This means how many samples are processed in a parallelized manner in one Kraken2 job (e.g., 400 samples with 128 cores given to KRAKEN2, then use 128 as batch size. This will generate 4 batches of samples, each an independent Kraken2 job running all samples in parallel at the same time. If 25 samples and 128 cores, then use 25 as batch size. This will result in 1 batch.)
