...
n_reads<-append(n_reads,system(paste0("samtools view ",contig,".bam '",output_contig$CONTIG[i],"':",output_contig$START[i],"-",output_contig$END[i]," | wc -l"),intern=TRUE))
system("export LC_NUMERIC=en_US.utf-8")
micr<-system(paste0("samtools view ",contig,".bam '",output_contig$CONTIG[i],"':",output_contig$START[i],"-",output_contig$END[i]," | cut -f1 | cut -f1,2 -d '_' | sort | uniq -c | sort -nr -k1,1 | head -5 | awk '{print $1\"_reads_\"$2}'"),intern=TRUE)
...
n_reads<-append(n_reads,system(paste0("samtools view ",contig,".bam '",output_contig$CONTIG[i],"':",output_contig$START[i],"-",output_contig$END[i]," | wc -l"),intern=TRUE))
#system("export LC_NUMERIC=en_US.utf-8")
micr<-system(paste0("samtools view ",contig,".bam '",output_contig$CONTIG[i],"':",output_contig$START[i],"-",output_contig$END[i]," | cut -f1 | sed 's/fna_/fna%/g' | cut -f1 -d '%' | sort | uniq -c | sort -nr -k1,1 | head -5 | awk '{print $1\"_reads_\"$2}'"),intern=TRUE)
micr1<-append(micr1,micr[1]); micr2<-append(micr2,micr[2]); micr3<-append(micr3,micr[3]); micr4<-append(micr4,micr[4]); micr5<-append(micr5,micr[5])
}
n_reads<-as.numeric(n_reads)
output_contig$N_READS<-n_reads
